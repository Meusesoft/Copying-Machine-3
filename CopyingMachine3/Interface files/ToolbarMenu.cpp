...
CMenuToolbar::CMenuToolbar(sGlobalInstances poGlobalInstances, CMenuIcon* poMenuIcon): CToolbar(poGlobalInstances)
{
	hWnd = NULL;
	hMenu = NULL;
	hPreviousFocus = NULL;
...
void CMenuToolbar::Attach(HINSTANCE phResource, LPCTSTR lpMenuName) {
	...
	if (hWnd==NULL) {
		...
		oTrace->EndTrace(__WFUNCTION__, (bool)false);
		return;
		}
...
long CMenuToolbar::OpenSubMenu(UINT nChar) {
	...
	for (long lIndex=0; lIndex<lMenuCount; lIndex++) {
		...
		if (cPointer>=0 && toupper(nChar)==towupper(cCharacter)) {
			...
			oTrace->EndTrace(__WFUNCTION__, lIndex, eAll);
			return lIndex;
			}
		}

	oTrace->EndTrace(__WFUNCTION__, -1, eAll);
	return -1;
}
...
void CMenuToolbar::OpenSubMenu(long plIndex) {
	...
	while (lNextMenuOpen!=-1) {
		...
		oMenuIcon->ProcessMenu(hSubMenu);
